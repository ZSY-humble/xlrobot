...
def resolve_robot_id(port):
    if port not in PORT_TO_ROBOT_ID:
        known_ports = ", ".join(f"{name} -> {robot_id}" for name, robot_id in PORT_TO_ROBOT_ID.items())
        raise ValueError(f"Unsupported port '{port}'. Known mappings: {known_ports}")

    return PORT_TO_ROBOT_ID[port]
...
def main():
    ...
    try:
        ...
        robot_id = resolve_robot_id(port)
        print(f"Using calibration id: {robot_id}")
