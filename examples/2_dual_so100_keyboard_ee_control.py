...
def main():
    ...
    try:
        ...
        arm1_id = PORT_TO_ROBOT_ID[arm1_port]
        arm2_id = PORT_TO_ROBOT_ID[arm2_port]
        
        print(f"Configuring first arm: {arm1_port}")  
        print(f"Configuring second arm: {arm2_port}")
        print(f"First arm calibration id: {arm1_id}")
        print(f"Second arm calibration id: {arm2_id}")
        
        # Create dual-arm robot instances
        arm1_config = SO100FollowerConfig(port=arm1_port, id=arm1_id)
        arm2_config = SO100FollowerConfig(port=arm2_port, id=arm2_id)
