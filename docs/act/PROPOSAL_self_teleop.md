...
| **头部 2 电机** | port1（与左臂同 bus） | bus1 | ✅ 开启 | **状态录入数据集**（保持不动） |
| **底盘 3 轮子** | port2（与右臂同 bus） | bus2 | — | 不使用 |
| **桌面相机** `top` | 外部三脚架 | - | - | **录入** |
...
| 字段 | 维度 | 含义 |
|---|---|---|
| `observation.state` | **8** | 右臂 6 关节 `pos` + 头部 2 电机 `pos`（合并成一维向量） |
